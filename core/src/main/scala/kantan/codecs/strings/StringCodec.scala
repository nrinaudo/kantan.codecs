...
object StringCodec {
  ...
  /** Creates a [[StringCodec]] instance for `java.util.Date`.
    ...
    *
    * scala> implicit val codec = StringCodec.dateCodec(new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSSz"))
    *
    * scala> StringDecoder[Date].decode("2016-01-17T22:03:12.012UTC")
    * res1: kantan.codecs.Result[DecodeError, Date] = Success(Sun Jan 17 23:03:12 CET 2016)
    *
    * // Encoding example
    * scala> StringEncoder[Date].encode(new Date(0))
