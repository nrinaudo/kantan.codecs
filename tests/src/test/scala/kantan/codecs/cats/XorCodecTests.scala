...
class XorCodecTests extends FunSuite with GeneratorDrivenPropertyChecks with Discipline {
  checkAll("StringDecoder[Int Xor Boolean]", DTests[String, Int Xor Boolean, DecodeError, codecs.type]
    .decoder[Int, Int])
  checkAll("StringEncoder[Int Xor Boolean]", ETests[String, Int Xor Boolean, codecs.type].encoder[Int, Int])
  checkAll("StringCodec[Int Xor Boolean]", CTests[String, Int Xor Boolean, DecodeError, codecs.type].codec[Int, Int])
}
